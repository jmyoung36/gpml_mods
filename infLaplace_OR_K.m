function [post nlZ dnlZ] = infLaplace_OR_K(hyp, meanfunc, cov, lik, K_cell, y)

% Laplace approximation to the posterior Gaussian process for ordinal regression.
...
%
% Modified by Orla Doyle to enable compatibility with ordinal regression
%
...
% using data in input space - don't use
% n = size(x,1);
% K = feval(cov{:},  hyp.cov,  x);                % evaluate the covariance matrix
% m = feval(mean{:}, hyp.mean, x);                      % evaluate the mean vector

% version of the above using precomputed kernel covariances
...
if any(size(last_alpha)~=[n,1])     % find a good starting point for alpha and f
  ...
  [lp,dlp,d2lp] = feval(lik,hyp.lik,y,f,[],inf); lp = -sum(lp);  W = -d2lp;  Psi_new = -lp;

else
  alpha = last_alpha; f = K*alpha+m;                              % try last one
  [lp,dlp,d2lp] = feval(lik,hyp.lik,y,f,[],inf);  lp = -sum(lp); W = -d2lp;
  Psi_new = alpha'*(f-m)/2 - lp;                      % objective for last alpha
  Psi_def = -sum(feval(lik,hyp.lik,y,m,[],inf));    % objective for default init f==m
...
[lp,dlp,d2lp,d3lp] = feval(lik,hyp.lik,y,f,[],inf); lp = -sum(lp);W = -d2lp; isWneg =any(W<0);
...
if nargout>2                                           % do we want derivatives?
  ...
  for j=1:length(hyp.mean)                                         % mean hypers
      % replace with precomputed kernel version
       ...
       % mean is constant - derivative wrt itself is a vector of ones
       % should increase generality by creating new version of mean
       % functions and actually evaluating them...
       dm = ones(n,1); 
       
       % have done this now
       dm = feval(meanfunc, hyp.mean, K_cell, j);
